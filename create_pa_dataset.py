...
def create_pa_only_dataset(source_path, dest_path, pa_label=7, only_pa_cases=True):
    """
    Create a new dataset with only pulmonary artery segmentations.
    
    Parameters:
    -----------
    source_path : str
        Path to the original CHD68 dataset
    dest_path : str
    ...
    print("\n" + "="*50)
    print("PA-Only Dataset Creation Summary")
    print("="*50)
    ...
    with open(os.path.join(dest_path, "README.txt"), "w") as f:
        f.write("Pulmonary Artery Segmentation Dataset\n")
        f.write("="*40 + "\n\n")
        f.write(f"This dataset is derived from the CHD68 segmentation dataset.\n")
...
def calculate_dataset_statistics(dest_path, pa_label=7):
    ...
    print("\n" + "="*50)
    print("PA-Only Dataset Statistics")
    print("="*50)
...
def main():
    parser = argparse.ArgumentParser(description="Create a Pulmonary Artery-only segmentation dataset from CHD68 dataset")
    parser.add_argument("--source", type=str, default="DATA/CHD68_segmentation_dataset/CHD68_segmentation_dataset_miccai19",
                        help="Path to the original CHD68 dataset")
    parser.add_argument("--dest", type=str, default="DATA/CHD68_PA_only_dataset",
